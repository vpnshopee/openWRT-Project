...
sed -i "s/uci set system.@system[0].timezone=CST-8/uci set system.@system[0].hostname=NanopiR2C\nuci set system.@system[0].timezone=MYT-8/g" zzz-default-settings
...
# Update Version
sed -i "/FriendlyWRT/a\ VPNshopee Build" package/base-files/files/etc/banner
#sed -i 's/Newifi D2/NANOPI R2C/g' files/etc/banner
...
sed -i "s/OpenWrt/Built by VPNshopee($(date +%Y.%m.%d)) @ OpenWrt/g" package/lean/default-settings/files/zzz-default-settings
